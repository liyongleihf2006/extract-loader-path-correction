...
function extractLoader(content) {
    ...
    var publicPath = options.publicPath === undefined ? this.options && this.options.output ? this.options.output.publicPath : undefined : options.publicPath;
    var dependencies = [];
    ...
    Promise.all(dependencies.map(loadModule, this)).then(function (sources) {
        ...
        if (_this.loaders[_this.loaderIndex - 1].options && /\[path\]/.test(_this.loaders[_this.loaderIndex - 1].options.name)) {
            results = results.map(function (result) {
                return (0, _loaderUtils.urlToRequest)(_path2.default.relative(_path2.default.relative(_this.rootContext || _this.options.context, _this.context), result));
            });
        }
